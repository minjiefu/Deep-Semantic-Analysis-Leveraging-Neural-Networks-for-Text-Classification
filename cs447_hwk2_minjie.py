# -*- coding: utf-8 -*-
"""cs447_hwk2_minjie.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1YXmCSCseTojQaiI13RV1cuKRAfBHljhI
...
class CbowDataset(data.Dataset):
    ...
    def make_training_examples(self):
        ...
        for sentence in self.sentences:
          for i in range(c, len(sentence) - c):
            target_word = sentence[i]
            context_before = sentence[i - c : i]
            context_after = sentence[i + 1 : i + 1 + c]
    ...
    def __getitem__(self, idx):
        ...
        target_word_index = self.get_target_index(idx)

        return context_vector, target_word_index
...
class CbowModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        hidden_output = F.relu(self.linear1(avg_embeds))
...
class TextDataset(data.Dataset):
    ...
    def convert_text(self):
        ...
        unk_idx = self.word2idx[CNN_UNK]
        end_idx = self.word2idx[CNN_END]
        ...
        for example in self.examples:
            ...
            for word in word_list:
              word_lower = word.lower()
              idx = self.word2idx.get(word_lower, unk_idx)
              review_indices.append(idx)
            review_indices.append(end_idx)
    ...
    def get_text(self, idx):
        ...
        pad_idx = self.word2idx[CNN_PAD]
        review_ids = self.textual_ids[idx]
        max_len = self.max_len

        if len(review_ids) > max_len:
            final_ids = review_ids[:max_len]
        else:
            padding_needed = max_len - len(review_ids)
            padding = [pad_idx] * padding_needed
            final_ids = review_ids + padding

        return torch.LongTensor(final_ids)
    ...
    def get_label(self, idx):
        ...
        ##### TODO #####
        label_str = self.examples[idx][0]
        label_int = 1 if label_str == 'pos' else 0

        return torch.LongTensor([label_int]).squeeze(0)
    ...
    def __getitem__(self, idx):
        ...
        ##### TODO #####
        text_vector = self.get_text(idx)
        label_vector = self.get_label(idx)
...
class CNN(nn.Module):
    def __init__(self, vocab_size, embed_size, out_channels, filter_heights, stride, dropout, num_classes, pad_idx):
        ...
        self.num_cnn_layers = len(filter_heights)
        ...
        input_linear_size = out_channels * self.num_cnn_layers
        self.fc = nn.Linear(input_linear_size, num_classes)
    ...
    def forward(self, texts):
        ...
        for conv in self.convs:
            conved = conv(embedded)
            conved = F.relu(conved)

            # c) Max-Pooling over Time: Take the max value across the sequence dimension (dim=2)
            # This is also called Global Max Pooling for this 1D case.
            # Shape: [batch_size, out_channels, seq_len_after_conv] -> [batch_size, out_channels, 1]
            pooled = F.max_pool1d(conved, kernel_size=conved.shape[2]).squeeze(2)
            # .squeeze(2) changes shape from [batch_size, out_channels, 1] to [batch_size, out_channels]
        ...
        # Apply dropout
        dropped_output = self.dropout_layer(concatenated_output)
